...
2. It fixes one cell of an exchange across the boundary: which end does it; what it carries (a number or a threshold; a name or a message a user sees); what is inside and outside; what happens when it is not there or fails. A verdict other than `PASS` and an open finding fix the cell their criterion names; a finding whose fix is already in the range is `logged`. A module split, an algorithm, and a format or a name that only code inside the boundary reads fixes no cell; a format, a reply or a protocol between programs, even across the boundary, fixes no cell either, since the owner's cells are the four named above; what must hold across units inside the boundary is a rule in the project's REVIEW.md or a constraint on a memory page, walked by the reviewer; it is not a row for the owner.
