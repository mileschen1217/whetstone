...
| AC-3 | DISPUTED (UNVERIFIED) | `bash .whetstone/epics/two-layers/units/boundary/checks/ac3.sh` | exit -: not run | needs a target: run the check there and record evidence/AC-3.log;  the check reads the builder's label at the end of each decision line ("a stored format", from decisions.md), and two labelled rows above the line in both runs are behaviours when a thing is absent or already there (`score` prints null when the key has no incident for the snapshot; `run` removes an earlier run's files in `--out` before it starts), which `line.md` names as an owner's cell; the page follows line.md, the check follows the label. The count half of AC-3 (4 or fewer) is 5 in run 1 and 4 in run 2 and is not disputed. |
| AC-4 | UNVERIFIED | `bash .whetstone/epics/two-layers/units/boundary/checks/ac4.sh` | exit -: not run | needs a target: run the check there and record evidence/AC-4.log |
| AC-5 | UNVERIFIED | `bash .whetstone/epics/two-layers/units/boundary/checks/ac5.sh` | exit -: not run | needs a target: run the check there and record evidence/AC-5.log |
| AC-6 | PASS | `bash .whetstone/epics/two-layers/units/boundary/checks/ac6.sh` | exit 0: evidence/AC-6.log at 05c3f1e | from recorded evidence, not run here |
