...
0.1.0, under construction. Two skills exist: `review`, for diffs only, and `build`, with `scripts/verify.sh`. Its measured results are in `evals/BASELINE.md`, with what was tried and dropped, what is not measured, and one known gap: in some runs the skill is not picked up from a plain request, and the review then behaves as if the plugin were absent. Invoke it by name. `intent`, `brief` and `ship` are not written; until `brief` exists a `brief.md` has to be written by hand in the shape `evals/build-reservations/fixture.sh` shows. See `CHANGELOG.md`.
